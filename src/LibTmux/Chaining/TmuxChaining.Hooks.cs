...
// The two hook requests that are not one command against one target: running
// or removing a hook are two verbs for one request, and setting several
// entries is a clear followed by one command per entry.
