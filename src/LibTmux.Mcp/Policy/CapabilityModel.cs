...
internal sealed class CapabilityRegistry
{
    ...
    private static ImmutableArray<ToolDefinition> BuildDefinitions()
    {
        ...
        return
        [
            Inspect("list_sessions", "List sessions", Metadata, nameof(CapabilityTools.ListSessionsAsync), S(), batchEligible: true),
            Inspect("list_windows", "List windows", Metadata, nameof(CapabilityTools.ListWindowsAsync), S(("session", InputSink.TmuxLookup)), batchEligible: true),
            Inspect("list_panes", "List panes", Metadata, nameof(CapabilityTools.ListPanesAsync), S(("session", InputSink.TmuxLookup), ("windowId", InputSink.TmuxLookup)), batchEligible: true),
            Inspect("get_server_info", "Get server info", Metadata, nameof(CapabilityTools.GetServerInfoAsync), S(), batchEligible: true),
            ...
            Inspect("wait_for_text", "Wait for text", PaneOutput, nameof(CapabilityTools.WaitForTextAsync), S(("paneId", InputSink.TmuxLookup), ("patterns", InputSink.Regex), ("stopPatterns", InputSink.Regex), ("timeoutSeconds", InputSink.None), ("ignoreCase", InputSink.None)), terminal: true, selfBounded: true, detail: "Wait for a pane to print something, instead of polling a capture. Only output arriving AFTER the call counts: text already on screen never matches, so a pattern visible in the returned tail can still time out."),
            Inspect("get_tmux_variables", "Get tmux variables", Configuration, nameof(CapabilityTools.GetTmuxVariablesAsync), SM(M("names", InputSink.TmuxLookup, InputSink.TmuxFormat), M("paneId", InputSink.TmuxLookup)), configured: true, batchEligible: true, inputLiteralization: F(("names", "validated-variable-name"))),
            Inspect("show_option", "Show option", Configuration, nameof(CapabilityTools.ShowOptionAsync), S(("name", InputSink.TmuxLookup), ("scope", InputSink.None), ("paneId", InputSink.TmuxLookup)), configured: true, untrusted: true, batchEligible: true),
            Inspect("show_environment", "Show environment", Environment, nameof(CapabilityTools.ShowEnvironmentAsync), S(("name", InputSink.TmuxLookup), ("session", InputSink.TmuxLookup)), environment: true, secrets: true, batchEligible: true, metadata: false),
            Inspect("show_hooks", "Show hooks", Configuration, nameof(CapabilityTools.ShowHooksAsync), S(("scope", InputSink.None), ("paneId", InputSink.TmuxLookup)), configured: true, untrusted: true, batchEligible: true, metadata: false),
            ...
            ManageTool("rename_session", "Rename session", Manage, nameof(CapabilityTools.RenameSessionAsync), SM(M("name", InputSink.TmuxState, InputSink.TmuxFormat), M("session", InputSink.TmuxLookup)), inputLiteralization: F(("name", "double-hash-once"))),
            ManageTool("rename_window", "Rename window", Manage, nameof(CapabilityTools.RenameWindowAsync), SM(M("name", InputSink.TmuxState, InputSink.TmuxFormat), M("windowId", InputSink.TmuxLookup)), inputLiteralization: F(("name", "double-hash-once"))),
            ManageTool("select_window", "Select window", Manage, nameof(CapabilityTools.SelectWindowAsync), S(("windowId", InputSink.TmuxLookup))),
            ManageTool("select_pane", "Select pane", Manage, nameof(CapabilityTools.SelectPaneAsync), S(("paneId", InputSink.TmuxLookup))),
            ManageTool("select_layout", "Select layout", Manage, nameof(CapabilityTools.SelectLayoutAsync), S(("windowId", InputSink.TmuxLookup), ("layout", InputSink.TmuxState))),
            ...
            ManageTool("resize_pane", "Resize pane", Manage, nameof(CapabilityTools.ResizePaneAsync), S(("paneId", InputSink.TmuxLookup), ("width", InputSink.None), ("height", InputSink.None), ("zoom", InputSink.None))),
            ManageTool("move_window", "Move window", Manage, nameof(CapabilityTools.MoveWindowAsync), S(("windowId", InputSink.TmuxLookup), ("destination", InputSink.TmuxState), ("session", InputSink.TmuxLookup), ("replaceExisting", InputSink.None))),
            ...
            ManageTool("set_pane_title", "Set pane title", Manage, nameof(CapabilityTools.SetPaneTitleAsync), SM(M("title", InputSink.TmuxState, InputSink.TmuxFormat), M("paneId", InputSink.TmuxLookup)), inputLiteralization: F(("title", "double-hash-once"))),
            ManageTool("enter_copy_mode", "Enter copy mode", Manage, nameof(CapabilityTools.EnterCopyModeAsync), S(("paneId", InputSink.TmuxLookup))),
            ManageTool("exit_copy_mode", "Exit copy mode", Manage, nameof(CapabilityTools.ExitCopyModeAsync), S(("paneId", InputSink.TmuxLookup))),
            ManageTool("wait_for_channel", "Wait for channel", Manage, nameof(CapabilityTools.WaitForChannelAsync), S(("channel", InputSink.TmuxState), ("timeoutSeconds", InputSink.None)), selfBounded: true, stateOnly: true),
            ManageTool("signal_channel", "Signal channel", Manage, nameof(CapabilityTools.SignalChannelAsync), S(("channel", InputSink.TmuxState)), stateOnly: true),
            ManageTool("set_mouse_enabled", "Set mouse enabled", Manage, nameof(CapabilityTools.SetMouseEnabledAsync), S(("enabled", InputSink.TmuxState)), stateOnly: true),
            ManageTool("set_history_limit", "Set history limit", Manage, nameof(CapabilityTools.SetHistoryLimitAsync), S(("lines", InputSink.TmuxState), ("session", InputSink.TmuxLookup)), stateOnly: true),
            ...
            Execute("run_shell_command", "Run shell command", Command, ProcessReach.PaneCommand, nameof(CapabilityTools.RunShellCommandAsync), SM(M("command", InputSink.PaneInput, InputSink.ShellCommand), M("paneId", InputSink.TmuxLookup), M("timeoutSeconds", InputSink.None), M("maxLines", InputSink.None), M("suppressHistory", InputSink.None)), terminal: true, detail: "Run a command in a pane, wait for it to finish, and report its real exit status and output. This is the tool for 'run X and tell me if it worked'. Do NOT send keys and then poll a capture. The command runs in a subshell, so cd and export do not persist. Check linesMissed and anchorLost."),
            Execute("send_keys", "Send keys", Input, ProcessReach.PaneInput, nameof(CapabilityTools.SendKeysAsync), S(("keys", InputSink.PaneInput), ("paneId", InputSink.TmuxLookup), ("enter", InputSink.PaneInput), ("literal", InputSink.None), ("suppressHistory", InputSink.None)), detail: "Send raw keystrokes to a pane and return immediately. Use for driving an interactive program — a key in vim, a menu choice, Ctrl-C. Set literal=false for named keys such as C-c, Escape or F5. To run a shell command and learn whether it worked, use run_shell_command instead; this reports nothing about what happens next."),
            Execute("send_keys_batch", "Send keys batch", Input, ProcessReach.PaneInput, nameof(CapabilityTools.SendKeysBatchAsync), SM(M("operations", InputSink.PaneInput, InputSink.TmuxLookup), M("onError", InputSink.None))),
            Execute("paste_text", "Paste text", Input, ProcessReach.PaneInput, nameof(CapabilityTools.PasteTextAsync), S(("text", InputSink.PaneInput), ("paneId", InputSink.TmuxLookup), ("bracketed", InputSink.None))),
    ...
    private static ToolDefinition ManageTool(
        ...
        FrozenDictionary<string, string>? inputLiteralization = null,
        bool selfBounded = false,
        bool stateOnly = false) => new(
            name, title, opener + " " + title + ".", Toolset.Manage, ProcessReach.None,
    ...
    private static ToolDefinition TeardownTool(
        ...
        string method,
        FrozenDictionary<string, ImmutableHashSet<InputSink>> sinks,
        bool observes = false) => new(
            name, title, opener + " " + title + ".", Toolset.Teardown, ProcessReach.None,
