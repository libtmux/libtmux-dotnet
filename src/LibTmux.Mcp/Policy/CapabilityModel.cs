...
internal sealed class CapabilityRegistry
{
    ...
    private static ImmutableArray<ToolDefinition> BuildDefinitions()
    {
        ...
        [
            ...
            Execute("run_shell_command", "Run shell command", Command, ProcessReach.PaneCommand, nameof(CapabilityTools.RunShellCommandAsync), SM(M("command", InputSink.PaneInput, InputSink.ShellCommand), M("paneId", InputSink.TmuxLookup), M("timeoutSeconds", InputSink.None), M("maxLines", InputSink.None), M("suppressHistory", InputSink.None)), terminal: true, detail: "Run a shell command in one configured effective pane, wait for it to finish, and report its singular real exit status and output. This is the tool for 'run X and tell me if it worked'. It refuses before baseline or payload when synchronize-panes would make the cohort larger. Do NOT send keys and then poll a capture in a loop — this waits deterministically and costs one call. The command runs in a subshell, so cd and export do not persist. It refuses the named pane in a human-owned mode. Check linesMissed and anchorLost. A timed-out command MAY STILL BE RUNNING; inspect it and do not retry it."),
