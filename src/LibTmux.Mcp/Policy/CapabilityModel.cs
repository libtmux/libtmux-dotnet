...
internal sealed class CapabilityRegistry
{
    ...
    private static ImmutableArray<ToolDefinition> BuildDefinitions()
    {
        ...
        [
            ...
            ManageTool("break_pane", "Break pane", Manage, nameof(CapabilityTools.BreakPaneAsync), S(("paneId", InputSink.TmuxLookup), ("detach", InputSink.None)), detail: "Move a pane out into a window of its own, and answer that window's id. Use it to promote a split that turned out to matter — a long build or a server — so it stops competing for room with the pane it was split from. The pane keeps its id and whatever is running in it. tmux names the window; rename_window takes the id this returns. A pane already alone in its window is refused, because it has one."),
            ManageTool("join_pane", "Join pane", Manage, nameof(CapabilityTools.JoinPaneAsync), S(("paneId", InputSink.TmuxLookup), ("targetPaneId", InputSink.TmuxLookup), ("direction", InputSink.None), ("detach", InputSink.None)), detail: "Move a pane into the window holding another pane, beside it. The reverse of break_pane: the pane keeps its id and its process, and the window it came from closes when its last pane leaves. Name the destination by a pane in it, because that also says where in the layout it lands."),
