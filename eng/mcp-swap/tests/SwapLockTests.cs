...
public sealed class SwapLockTests
{
    ...
    public async Task ValidationAndAnOpenedAliasDoNotReleaseTheNativeRecordLock()
    {
        ...
        // Disposal validates, and a lock this test left aliased is invalid, so
        // letting disposal's exception replace the body's hides which one
        // actually failed. It is released only by Dispose and the semaphore it
        // releases is process-wide, so it still has to run either way.
        ...
        finally
        {
            ...
            catch (IOException) when (bodyFailure is not null)
            {
            }
        }
    }
