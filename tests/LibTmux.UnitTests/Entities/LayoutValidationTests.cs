...
public sealed class LayoutValidationTests
{
    ...
    public async Task Direct_and_chained_names_are_refused_before_any_mutation(bool chained)
    {
        ...
        Window window = new(server, server.Connection!, Generation, new WindowId(1), new Dictionary<string, string?>
        {
            ["window_id"] = "@1",
            ["session_id"] = "$1",
        });
        var request = new SelectLayoutRequest { Layout = "main-horizontal-mirrored" };
    ...
    public async Task A_chain_validates_once_through_its_execution_connection()
    {
        ...
        Window window = new(original, original.Connection!, Generation, new WindowId(1), new Dictionary<string, string?>
        {
            ["window_id"] = "@1",
            ["session_id"] = "$1",
        });
        TmuxCommand command = new SelectLayoutRequest { Layout = "main-horizontal-mirrored" }.ToCommand(window);
        int queries = 0;
    ...
    public void Typed_layout_guard_participates_in_command_equality_and_raw_replacement()
    {
        ...
        TmuxCommand typed = new SelectLayoutRequest { Layout = "main-h" }.ToCommand(window);
        TmuxCommand same = new SelectLayoutRequest { Layout = "main-h" }.ToCommand(window);
