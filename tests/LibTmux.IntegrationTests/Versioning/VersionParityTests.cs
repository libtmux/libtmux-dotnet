...
public sealed class VersionParityTests
{
    ...
    public async Task LayoutMirrors()
    {
        ...
        try
        {
            Session session = await server.CreateSessionAsync(new NewSessionRequest("layout-proof"), token);
            ...
            window = await window.SelectLayoutAsync(new SelectLayoutRequest(accepted), token);
            ...
            TmuxWindowException failure = await Assert.ThrowsAsync<TmuxWindowException>(() =>
                window.SelectLayoutAsync(new SelectLayoutRequest(rejected), token));
