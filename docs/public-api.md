...
| `M:LibTmux.Server.ValidateLayoutsAsync(IEnumerable<(string Layout,int PaneCount)>,CancellationToken)` | `Task LibTmux.Server.ValidateLayoutsAsync(IEnumerable<(string Layout, int PaneCount)> layouts, CancellationToken cancellationToken = default)` | Public | No | `UnsupportedOSPlatform("windows")` | Checks layout syntax and minimum cell counts before mutation, resolving version-sensitive names against the daemon or an authenticated cold client. |
