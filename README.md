...
| Trimming / NativeAOT | `LibTmux` and `LibTmux.Query.Json` are analyzer-gated, and one smoke app referencing both is published and run ahead of time for `linux-x64` on net8.0 and net10.0. `Compile` and `Matching` resolve properties by name, so they warn trimmed callers to preserve the filtered types' public properties. `LibTmux.Workspace` sets `IsAotCompatible=false` because its NativeAOT closure is unproven; the proof does not cover macOS or native Windows/psmux |
